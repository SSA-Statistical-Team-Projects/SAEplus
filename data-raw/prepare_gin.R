...
# gin_mastercentroid.dt[, (new_vars) := lapply(.SD, weighted.mean, w = population, na.rm = TRUE), by = "ADM3_CODE", .SDcols = vars]
gin_mastercentroid.dt[, (new_vars) := lapply(.SD, mean, na.rm = TRUE), by = "ADM3_CODE", .SDcols = vars]

states <- c("Conakry", "Kankan", "Nzerekore",
            "Faranah", "Labe", "Mamou", "Kindia", "Boke")
state_vars <- unlist(lapply(states, append_to_names))
gin_mastercentroid.dt[, (state_vars) := lapply(.SD, mean, na.rm = TRUE), by = "ADM1_CODE", .SDcols = state_vars]

gin_masterpoly.dt <- as.data.table(gin_masterpoly.dt)

missing_vars <- vars[!(new_vars %in% vars)]
gin_masterpoly.dt <- gin_mastercentroid.dt[, c(new_vars, "id", missing_vars), with = FALSE][gin_masterpoly.dt, on = "id"]
...
##write up function to clean up admin code

relab_admin <- function(dt){
  dt <- as.data.table(dt)
  dt[,ADM3_CODE := as.integer(substr(ADM3_CODE, 4, nchar(ADM3_CODE)))]
  dt[,ADM2_CODE := as.integer(substr(ADM2_CODE, 4, nchar(ADM2_CODE)))]
  dt[,ADM1_CODE := as.integer(substr(ADM1_CODE, 4, nchar(ADM1_CODE)))]
  return(dt)
}

hh.dt <- relab_admin(hh.dt)
gin_master.dt <- relab_admin(gin_master.dt)
gin_mastercentroid.dt <- relab_admin(gin_mastercentroid.dt)
...
selected.vars <- SAEplus::saeplus_selectmodel(dt = gin_master.dt,
                                              var_identifier = c("bld_", "_2018", "_2019", "adm",
                                                                 "rwi", "Conakry", "Kankan", "Nzerekore",
                                                                 "Faranah", "Labe", "Mamou", "Kindia", "Boke",
                                                                 "coverfraction"),
                                              method = "LASSO")
...
selected.vars <- c(selected.vars, "Conakry")
