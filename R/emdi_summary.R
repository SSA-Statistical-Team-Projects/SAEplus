...
summary.ebp <- function(object, ...) {
  ...
  if (length(residuals(object$model$model, level = 0, type = "pearson")) > 3 &
    ...
    shapiro_p_res <-
      shapiro.test(residuals(object$model$model, level = 0, type = "pearson"))[[2]]
    shapiro_W_res <-
      shapiro.test(residuals(object$model$model, level = 0, type = "pearson"))[[1]]
  } else {
  ...
  if (length(ranef(object$model$model)$'(Intercept)') > 3 &
      length(ranef(object$model$model)$'(Intercept)') < 5000) {
    shapiro_p_ran <- shapiro.test(ranef(object$model$model)$'(Intercept)')[[2]]
    shapiro_W_ran <- shapiro.test(ranef(object$model$model)$'(Intercept)')[[1]]
  } else {
  ...
  icc_mixed <- icc(object$model$model)
