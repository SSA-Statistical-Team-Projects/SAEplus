...
my.framework_ebp <- function(fixed, pop_data, pop_domains, smp_data, smp_domains,
                             threshold, pop_weight = NULL, smp_weight = NULL, custom_indicator = NULL, na.rm) {
  ...
  emdi:::fw_check1(pop_data = pop_data, mod_vars = mod_vars, pop_domains = pop_domains,
  ...
  emdi:::fw_check2(pop_domains = pop_domains, pop_domains_vec = pop_domains_vec,
  ...
  emdi:::fw_check3(obs_dom = obs_dom, dist_obs_dom = dist_obs_dom, pop_domains = pop_domains,
  ...
  indicator_list <- list(
    ...
    # }
    # ,
    # qsr = function(y, threshold,censusweights) {
    #   weights <- rep.int(1, length(y))
    #   quant14 <- emdi:::wtd.quantile(x = y,
    #                         weights = weights,
    #                         probs = c(0.2, 0.8))
    #
    #   iq1 <- y <= quant14[1]
    ...
    #   },
    # quants = function(y, threshold, censusweights) {t(quantile(y, probs = c(.10,.25, .5, .75, .9)))}
  )
  ...
  return(list(pop_data         = pop_data,
              ...
              threshold         = threshold,
              popweight_data     = popweight_data,
              smpweight_data      = smpweight_data
  )
  )
}
